...
class DeepSeekChat:
    ...
    def send_message(self,message,printing=True,thinking_enabled=False,search_enabled=False):
                ...
                def parse_output(data,line):
                    ...
                    if type(data)==dict:
                        if generate_mode=='SEARCH' and 'url' in data:
                            ...
                            if printing:
                                print(f"#[{data.get('cite_index','?')}]: [{data.get('title',data.get('site_name','UNKNOWN'))} - {data.get('site_name','UNKNOWN')}]({data['url']})")
...
if __name__ == "__main__":
    ...
    response = chat.send_message(input('Your message:'),True,True,True)
    if response:
